...
function (angular, _, dateMath, moment) {
  ...
  function MetaQueriesDatasource($q, datasourceSrv) {
    ...
    function arithmetic(target, targetsByRefId, outputMetricName, results){
        ...
        var underlyQueryType = target.underlyQueryType;

        if(underlyQueryType === "Druid-topN"){
          return arithmeticForDruidTopN(target, targetsByRefId, outputMetricName, results);          
        }else if(underlyQueryType === "Druid-groupBy"){
          return arithmeticForDruidGroupBy(target, targetsByRefId, outputMetricName, results);
        }else{
          return arithmeticForRaw(target, targetsByRefId, outputMetricName, results);
        }
